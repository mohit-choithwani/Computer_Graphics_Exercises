...
mat4.perspective = function (out, fovy, near, far) {
    ...
    var aspect = 1;
    var r = aspect*near*Math.tan(fovy/2);
    var l = -r;
    var t = near*Math.tan(fovy/2);
    var b = -t;
    //S=1/Math.tan((fovy/2)*(Math.PI/180));
     out[0] = 2/(r-1);
     out[1] = 0;
     out[2] = 0;
     out[3] = 0;
...
class Camera3D {
    ...
    update() {
        ...
        // this.v = ?
        var g = [0, 0, 0];
        g[0] = this.lookAtPoint[0] - this.eye[0];
        g[1] = this.lookAtPoint[1] - this.eye[1];
        g[2] = this.lookAtPoint[2] - this.eye[2];
        console.log('norm of G-->'+(-g[0]/Math.sqrt(g[0]*g[0] + g[1]*g[1] + g[2]*g[2]))+'  '+(-g[1]/Math.sqrt(g[0]*g[0] + g[1]*g[1] + g[2]*g[2]))+'  '+(-g[2]/Math.sqrt(g[0]*g[0] + g[1]*g[1] + g[2]*g[2])));

	// ************** Defining the camera coordinates u,v,w **************

	//
	//************** w vector **************
  this.w = vec3.create();
  this.w[0] = -g[0]/Math.sqrt(g[0]*g[0] + g[1]*g[1] + g[2]*g[2]);
  this.w[1] = -g[1]/Math.sqrt(g[0]*g[0] + g[1]*g[1] + g[2]*g[2]);
  this.w[2] = -g[2]/Math.sqrt(g[0]*g[0] + g[1]*g[1] + g[2]*g[2]);
  console.log('W vector-->'+this.w);

  var crossproduct_t_and_w = vec3.create();
  vec3.cross(crossproduct_t_and_w,this.upVector,this.w);
  console.log('Cross Product of T and w--->'+crossproduct_t_and_w);
    //var norm_crossproduct_t_and_w = vec3.length(crossproduct_t_and_w);

  var norm_crossproduct_t_and_w = Math.sqrt(crossproduct_t_and_w[0]*crossproduct_t_and_w[0] + crossproduct_t_and_w[1]*crossproduct_t_and_w[1] + crossproduct_t_and_w[2]*crossproduct_t_and_w[2]);
  console.log('Norm of cross Product of T and w--->'+norm_crossproduct_t_and_w);

//------------------u vector----------------------------------------------------

	this.u = vec3.create();
	this.u[0] = crossproduct_t_and_w[0]/norm_crossproduct_t_and_w;
	this.u[1] = crossproduct_t_and_w[1]/norm_crossproduct_t_and_w;
	this.u[2] = crossproduct_t_and_w[2]/norm_crossproduct_t_and_w;

    var crossproduct_w_and_u = vec3.create();

    vec3.cross(crossproduct_w_and_u,this.w,this.u);

    var norm_crossproduct_w_and_u = Math.sqrt(crossproduct_w_and_u[0]*crossproduct_w_and_u[0] + crossproduct_w_and_u[1]*crossproduct_w_and_u[1] + crossproduct_w_and_u[2]*crossproduct_w_and_u[2])

    this.v = vec3.create();
    this.v[0] = crossproduct_w_and_u[0]/norm_crossproduct_w_and_u;
    this.v[1] = crossproduct_w_and_u[1]/norm_crossproduct_w_and_u;
    this.v[2] = crossproduct_w_and_u[2]/norm_crossproduct_w_and_u;
