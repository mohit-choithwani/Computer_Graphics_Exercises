...
function Basic1_1(canvas) {
    ...
    function OrthogonalProjection2D(point2D) {
        ...
        //Multiplying with [1 0] matrix result the 1D cordinate on x-axis
        resultant_point = 1*point2D[0] + 0*point2D[1]
...
function Basic1_2(canvas) {
    ...
    function PerspectiveProjection2D(eye, imagePlane, point2D) {
        ...
        // console.log('Eye position-->'+eye[0]+' & '+eye[1]);
        // console.log('point position-->'+point2D[0]+' & '+point2D[1]);
        // converting 2D point to Homogenous cordinate
        var point_x = [point2D[0], point2D[1], 1.0];

        // Defining matrix that transform world cordinate to camera cordinate
        var Transform_Matrix = [1, 0, -eye[0], 0, 1, -eye[1], 0, 0, 1];

        var point2D_new =  [ (Transform_Matrix[0]*point_x[0] + Transform_Matrix[1]*point_x[1] + Transform_Matrix[2]*1.0),
						                 (Transform_Matrix[3]*point_x[0] + Transform_Matrix[4]*point_x[1] + Transform_Matrix[5]*1.0),
							               (Transform_Matrix[6]*point_x[0] + Transform_Matrix[7]*point_x[1] + Transform_Matrix[8]*1.0) ] ;
